    ...
    <div className="card-minimal rounded-2xl p-6 space-y-6 font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-200/80 dark:border-neutral-800/80 pb-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl border border-purple-200/80 dark:border-purple-900/60 bg-purple-50/80 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400">
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] px-2.5 py-1 rounded-full border border-purple-300/80 dark:border-purple-700/80 bg-purple-50 dark:bg-purple-950/60 text-purple-800 dark:text-purple-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-neutral-200/80 dark:divide-neutral-800 rounded-xl border border-neutral-200/80 dark:border-neutral-800 bg-white dark:bg-surface-2 text-xs overflow-hidden shadow-2xs">
        {/* Step 1: Thermal Arrhenius Baseline */}
        <div className="p-4 space-y-2">
          <div className="text-[11px] text-neutral-500 uppercase tracking-wide flex items-center justify-between">
            <span>1. Thermal Baseline</span>
            <span className="font-mono text-[10px] text-neutral-400">Arrhenius / Q₁₀</span>
          </div>
          <div className="font-mono text-2xl font-bold text-neutral-950 dark:text-white">
            {(baseThermalDecay * 100).toFixed(1)}% <span className="text-[11px] font-normal text-neutral-400">spoilage</span>
          </div>
          <div className="text-[11px] text-neutral-500 dark:text-neutral-400">
            Thermal Shelf-Life: <strong className="font-mono text-neutral-900 dark:text-white">{baseShelfLifePct.toFixed(1)}%</strong> ({effectiveThermalHours.toFixed(1)} eff. hrs)
          </div>
        </div>

        {/* Step 2: PINN Mechanical Stress Layer */}
        <div className="p-4 space-y-2 bg-purple-50/30 dark:bg-purple-950/20">
          <div className="text-[11px] text-purple-600 dark:text-purple-400 uppercase tracking-wide flex items-center justify-between">
            <span>2. PINN Stress Layer</span>
            <span className="font-mono text-[10px] text-purple-500">Mechanical Multiplier</span>
          </div>
          <div className="font-mono text-2xl font-bold text-purple-700 dark:text-purple-300 flex items-baseline gap-2">
            <span>&times; {stressMultiplier.toFixed(2)}x</span>
            <span className="text-xs text-neutral-400 font-normal">multiplier</span>
          </div>
          <div className="text-[11px] text-neutral-500 dark:text-neutral-400">
            Fatigue Damage: <strong className="font-mono text-neutral-900 dark:text-white">{(pinnData?.mechanical_damage ? pinnData.mechanical_damage * 100 : 0).toFixed(1)}%</strong> &bull; {pinnData?.vibration_level || "Active"}
          </div>
        </div>

        {/* Step 3: Combined Final Spoilage Risk */}
        <div className="p-4 space-y-2 bg-rose-50/30 dark:bg-rose-950/20">
          <div className="text-[11px] text-rose-600 dark:text-rose-400 uppercase tracking-wide flex items-center justify-between">
            <span>3. Final Coupled Risk</span>
            <span className="font-mono text-[10px] text-rose-500">Coupled Kinetics</span>
          </div>
          <div className="font-mono text-2xl font-bold text-rose-700 dark:text-rose-300">
            {(finalCoupledDecay * 100).toFixed(1)}% <span className="text-[11px] font-normal text-neutral-400">total risk</span>
          </div>
          <div className="text-[11px] text-neutral-500 dark:text-neutral-400">
            Remaining Usable Life: <strong className="font-mono text-rose-600 dark:text-rose-400">{finalShelfLifePct}%</strong>
          </div>
        </div>
      </div>

      {/* Physics Formula Explanation */}
      <div className="p-4 rounded-xl border border-neutral-200/80 dark:border-neutral-800 bg-neutral-50/70 dark:bg-surface-2 text-xs space-y-1.5">
        <div className="flex flex-wrap items-center justify-between gap-1 text-[11px] text-neutral-500 dark:text-neutral-400">
          <span className="font-medium text-neutral-900 dark:text-white">Coupled Thermal-Mechanical Kinetics:</span>
          <span className="font-mono text-[10px] text-neutral-600 dark:text-neutral-300">FinalDecay = ThermalDecay &times; (1.0 + α·RMSᵝ·[1 + γ·ΔT])</span>
        </div>
        <p className="text-[11px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
          The Arrhenius/$Q_{10}$ model computes biological respiration and enzymatic decay. The PINN layer injects physical cyclic fatigue and structural bruising from road shock. When vibration telemetry is zero or disconnected, the multiplier falls back to exactly <strong className="font-mono text-neutral-950 dark:text-white">1.00x</strong> (zero added decay).
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          <div className="space-y-1">
            <div className="flex justify-between text-[11px] text-neutral-500 dark:text-neutral-400">
              ...
              <span className="font-mono font-medium text-neutral-900 dark:text-white">{sliderTemp}°C</span>
            </div>
            <input
              type="range"
              min={10}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-[11px] text-neutral-500 dark:text-neutral-400">
              ...
              <span className="font-mono font-medium text-neutral-900 dark:text-white">{sliderVibe.toFixed(2)} m/s²</span>
            </div>
            <input
              type="range"
              min={0.0}
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-[11px] text-neutral-500 dark:text-neutral-400">
              ...
              <span className="font-mono font-medium text-neutral-900 dark:text-white">{sliderDuration} hrs</span>
            </div>
            <input
              type="range"
              min={1}
